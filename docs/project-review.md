...
**17. ~~C++ 后端无单元测试~~ ✅ 已修复**
~~项目无任何测试。`calculateSpeed`、`readNetworkStats` 的正则解析、`isPhysicalInterface` 等纯逻辑函数适合且应该有单元测试覆盖。~~
已新建 `tests/tst_networkmonitorapplet.cpp`（Qt Test），5 个测试用例覆盖 `isPhysicalInterface`、速度计算、负值钳制、总量累加、接口校验。CMake 增加 `enable_testing()` + `add_test`，`ctest` 全部通过。
...
| # | 需求 | 说明 | 价值 |
|---|------|------|------|
| 18 | **日/月流量统计持久化** | 将每日累计流量写入 `settings.ini`，重启后保留。弹窗/设置窗口展示今日/本月用量 | 核心功能提升 |
| ~~19~~ | ~~深色模式适配~~ ✅ 随 #4 完成 | | |
| ~~20~~ | ~~公共代码抽取~~ ✅ 随 #11 完成 | | |
| ~~21~~ | ~~文档同步~~ ✅ 随 #15 完成 | | |
